#[derive(Debug, Clone, Copy)]
pub enum LogEvent {
    ServerListening,
    RequestReceived,
    TransparentProxyUsed,
    AdapterUsed,
    UpstreamResponseReceived,
    StreamEventConversionWarning,
    UpstreamError,
}

pub fn emit(event: LogEvent, message: impl AsRef<str>) {
    println!("[llmapi] {:?}: {}", event, message.as_ref());
}

pub fn redact_key(key: &str) -> String {
    let prefix: String = key.chars().take(6).collect();
    format!("{prefix}...")
}

pub fn redact_query(query: &str) -> String {
    url::form_urlencoded::parse(query.as_bytes())
        .map(|(key, value)| {
            if is_sensitive_key(&key) {
                format!("{key}={}", redact_key(&value))
            } else {
                format!("{key}={value}")
            }
        })
        .collect::<Vec<_>>()
        .join("&")
}

pub fn redact_url(url: &str) -> String {
    let Some((base, query)) = url.split_once('?') else {
        return url.to_string();
    };

    format!("{base}?{}", redact_query(query))
}

pub fn request_line(method: &str, uri: &axum::http::Uri) -> String {
    let path = uri.path();
    match uri.query() {
        Some(query) => format!("{method} {path}?{}", redact_query(query)),
        None => format!("{method} {path}"),
    }
}

fn is_sensitive_key(key: &str) -> bool {
    let key = key.to_ascii_lowercase();
    matches!(
        key.as_str(),
        "key" | "api_key" | "apikey" | "access_token" | "token"
    )
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn redacts_sensitive_query_values() {
        assert_eq!(
            redact_query("alt=sse&key=sk-123456&api_key=token-abcdef"),
            "alt=sse&key=sk-123...&api_key=token-..."
        );
    }

    #[test]
    fn request_line_keeps_path_and_redacts_query() {
        let uri = "/v1/chat/completions?key=sk-query&mode=test"
            .parse()
            .unwrap();

        assert_eq!(
            request_line("POST", &uri),
            "POST /v1/chat/completions?key=sk-que...&mode=test"
        );
    }
}
