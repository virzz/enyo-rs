...
fn decrypt_db(path: &PathBuf, pkey: &String, ver: &WechatDbType) -> Result<Vec<u8>> {
    ...
    unsafe {
        ...
        reserve = if reserve.is_multiple_of(AES_BLOCK_SIZE) {
            reserve
        } else {
        ...
        for cur_page in 0..total_page {
            ...
            let hash_mac = match ver {
                WechatDbType::V3 => {
                    ...
                    mac.update(
                        std::mem::transmute::<&u32, &[u8; 4]>(&(cur_page as u32 + 1)).as_ref(),
                    );
...
fn convert_to_sqlcipher_rawkey(pkey: &str, path: &PathBuf, ver: &WechatDbType) -> Result<String> {
    ...
    File::open(path)?.read_exact(salt.as_mut())?;
    let pass = hex::decode(pkey)?;
    match ver {
        WechatDbType::V3 => {
...
fn do_exec(
    ...
    match ver {
        WechatDbType::V3 => {
            conn.execute_batch(
                format!(
                    "PRAGMA kdf_iter = {ROUND_COUNT_V3};
        ...
        WechatDbType::V4 => {
            conn.execute_batch(
                format!(
                    "PRAGMA kdf_iter = {ROUND_COUNT_V4};
        ...
        WechatExecOutput::Json => {
            ...
            while let Some(row) = rows.next()? {
                ...
                for (i, name) in names.iter().enumerate().take(count) {
                    item.insert(name.clone(), Value::String(row.get::<usize, String>(i)?));
                }
                items.push(Value::Object(item));
            }
            println!("{}", to_colored_json_auto(&items)?);
        }
    }
    Ok(())
}
...
impl Action for Cmd {
    async fn execute(&self) -> Result<()> {
        ...
        let ver: WechatDbType = match self.ver.clone() {
            Some(v) => v,
            None => WechatDbType::V3,
        };
        if let Some(sql) = self.sql.clone() {
            let pkey = convert_to_sqlcipher_rawkey(&key, &file, &ver)?;
            let output = self.output.clone().unwrap_or(WechatExecOutput::Table);
